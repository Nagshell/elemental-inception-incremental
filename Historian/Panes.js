...
var panes = {
	...
	mouseHandler: function (event) {
		...
		stuckHelperPane.recentlyGrowing = stuckHelperPane.growing;
		stuckHelperPane.growing = false;
		...
		if (event.target.id !== "canvasMain") return;
		var x = event.offsetX;
		var y = event.offsetY;
...
cPane.prototype.draw = function (ctx) {
	...
	if (this.boundaryPath) {
		...
		if (this.title) {
			...
			ctx.fillText(
				this.title,
				optionData.iconSize * 3 + 5,
				optionData.iconSize / 2
			);
			ctx.lineWidth = 1;
			ctx.beginPath();
