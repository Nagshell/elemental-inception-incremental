...
// a lookup table for eventId to event object;
let events = {};

/* will take and eventId and dispatch an event for any game event using that ID
 *
 * NOTE: currently, since new events are created for every game event, only one event
 * will fire when you use renEvent. this is the same as the previous code. However,
 * you could easily tweak the code to allow for multiple GameEvents to use the same
 * event object, in which case you could make multiple events fire for any given eventId;
 */
...
/*
 * Simply creats a new event for an ID and adds an event listener for that event.
 */
function GameEvent(eventId, callback) {
	this.eventId = eventId;
	this.event = new Event(eventId);
	events[eventId] = this.event;
...
new GameEvent( "elementBase2", () => {
	sendMessage(1,"Air Orb's online.");
});

new GameEvent( "elementBase3", () => {
...
new GameEvent( "elementAmount8-0", () => {
	if(eventFlags[0]) {
		eventFlags[0] = false;
...
new GameEvent( "elementAmount8-2", () => {
	if(eventFlags[1]) {
		eventFlags[1] = false;
...
new GameEvent( "machineAmount3-0", () => {
	if(eventFlags[2]) {
		eventFlags[2] = false;
...
new GameEvent( "machineAmount3-1", () => {
	if(eventFlags[3]) {
		eventFlags[3] = false;
...
new GameEvent( "machineAmount3-2", () => {
	if(eventFlags[4]) {
		eventFlags[4] = false;
